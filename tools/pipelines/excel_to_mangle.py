...
Usage:
    python3 tools/pipelines/excel_to_mangle.py
    python3 tools/pipelines/excel_to_mangle.py --input docs/ --output data/mangle_facts/
"""
...
from paths import DOCS_DIR, MANGLE_FACTS_DIR, REPO_ROOT
...
def main():
    ...
    parser.add_argument(
        ...
        help="Input directory containing Excel files (default: repo docs/)",
    )
    parser.add_argument(
        "--output",
        default=str(MANGLE_FACTS_DIR),
        help="Output directory for .mangle files (default: data/mangle_facts/)",
    )
    args = parser.parse_args()

    # Resolve paths relative to the repository root
    input_dir = args.input if os.path.isabs(args.input) else str(REPO_ROOT / args.input)
    output_dir = args.output if os.path.isabs(args.output) else str(REPO_ROOT / args.output)
