import { PDFParse } from "../packages/parser/node_modules/pdf-parse/dist/pdf-parse/cjs/index.cjs";
...
const CATEGORY_META: Record<TaxCategory, TaxCategoryMeta> = {
  ...
  meals_entertainment: {
    ...
    deductible: "partial",
    deductionRate: 0.5,
    section: "S14(1) + S17 proviso",
    note: "50% deductible unless staff-only events (100%)",
  },
  office_supplies: {
  ...
  cash_withdrawal: {
    ...
    deductionRate: 0,
    section: "N/A",
    note: "Neutral — cash withdrawn, deductibility depends on actual use",
  },
  transfer_internal: {
...
const RULES: CategorisationRule[] = [
  // ─ Revenue / Inflows ─
  {
    patterns: [/IBG GIRO.*ISETAN/i, /IBG GIRO.*(?!CPF|IRAS)/i],
    category: "revenue",
    direction: "deposit",
  },
  {
    patterns: [/PAYMENT\/TRANSFER.*OTHR\s*S\$/i],
    category: "revenue",
    direction: "deposit",
  },
  {
    patterns: [/FUND TRANSFER.*via PayNow/i],
    category: "revenue",
    direction: "deposit",
  },
  {
    patterns: [/FAST PAYMENT/i],
    category: "revenue",
    direction: "deposit",
  },
  {
    patterns: [/INTEREST\s*CREDIT/i, /INT\s*EARNED/i],
    category: "revenue",
    direction: "deposit",
  },
  ...
  // ─ IRAS / Tax ─
  {
    patterns: [/IRAS/i, /INLAND\s*REVENUE/i, /TAX\s*PAYMENT/i, /GST\s*PAYMENT/i],
  ...
  // ─ Salary / Payroll ─
  {
    patterns: [
      ...
      /FAST PAYMENT.*OTHR\s+(?!VICTORIAENSO|IRINA)/i,
    ],
  ...
  {
    patterns: [
      /CHARGES/i, /BANK\s*FEE/i, /SERVICE\s*CHARGE/i,
      /SER\s*CHARGE/i, /CCY\s*CONVERSION\s*FEE/i, /TRAN\s*CHARGE/i,
      /Txn\s*Charges/i, /ANNUAL\s*FEE/i, /ACCOUNT\s*FEE/i,
    ],
  ...
  // ─ Insurance ─
  {
    patterns: [/INSURANCE/i, /INSUR/i, /AIA/i, /PRUDENTIAL/i, /GREAT EASTERN/i, /NTUC INCOME/i],
    category: "insurance",
  },
  ...
  // ─ Travel ─
  {
    patterns: [
      ...
      /AIRBNB/i, /AGODA/i, /EXPEDIA/i, /KLOOK/i,
      /AIRLINES?/i, /SCOOT/i, /JETSTAR/i, /CATHAY/i, /SIA/i,
      /TAXI|GRAB|GOJEK|COMFORT/i,
      /MRT|SMRT|TRANSIT/i, /EZ-?LINK/i,
    ],
  ...
  {
    patterns: [
      /SUPERMARKET/i, /NTUC/i, /FAIRPRICE/i, /COLD\s*STORAGE/i,
      /SHENG\s*SIONG/i, /GIANT/i, /DON\s*DON\s*DONKI/i,
      /TOP\s*CHOICE/i,
    ],
    category: "meals_entertainment",
  },
  ...
  // ─ Medical ─
  {
    patterns: [
      ...
      /DOCTOR/i, /DENTAL/i, /HEALTH/i, /MEDICAL/i,
  ...
  // ─ Utilities ─
  {
    patterns: [/SP\s*GROUP/i, /SP\s*SERVICES/i, /PUB\b/i, /SINGTEL/i, /STARHUB/i, /M1\b/i],
    category: "utilities",
  },

  // ─ Shopping / Personal (non-deductible) ─
  {
    patterns: [
      /SILVER\s*SHOP/i, /JEWEL/i, /KHUN\s*PANG/i,
    ],
    category: "personal_non_deductible",
  },

  // ─ Debit Purchase catch-all ─
  {
...
async function main() {
  const dirs = [
    { dir: "/Users/user/Downloads/c-s/2023", year: 2023 },
  ...
  console.log("  Period: January 2023 – December 2025");
  console.log("═".repeat(100));
  console.log();
  ...
  console.log(`
  1. UNCATEGORISED TRANSACTIONS (${uncategorised.length} items, S$${uncategorised.reduce((s, t) => s + (t.withdrawal ?? 0), 0).toLocaleString("en-SG", { minimumFractionDigits: 2 })})
     → Review these manually. Many may qualify as deductible business expenses.
     → Retain receipts and document business purpose for each.
`);

  console.log(`  2. CASH WITHDRAWALS (${cashWithdrawals.length} items, S$${cashWithdrawals.reduce((s, t) => s + (t.withdrawal ?? 0), 0).toLocaleString("en-SG", { minimumFractionDigits: 2 })})
     → Cash withdrawals are not deductible by default, but the underlying
       expenses may be. Keep a petty cash log with receipts to claim deductions.
`);

  console.log(`  3. MEALS & ENTERTAINMENT (${mealsItems.length} items, S$${mealsItems.reduce((s, t) => s + (t.withdrawal ?? 0), 0).toLocaleString("en-SG", { minimumFractionDigits: 2 })})
     → Currently at 50% deduction rate. Staff-only meals/events can be
       claimed at 100%. Document which meals are staff welfare vs client entertainment.
`);

  console.log(`  4. PERSONAL / NON-DEDUCTIBLE (${personalItems.length} items, S$${personalItems.reduce((s, t) => s + (t.withdrawal ?? 0), 0).toLocaleString("en-SG", { minimumFractionDigits: 2 })})
     → These are flagged as non-business. If any have a legitimate business
       purpose, reclassify them and document the business reason.
`);

  // Potential additional deductions
  const totalMeals = mealsItems.reduce((s, t) => s + (t.withdrawal ?? 0), 0);
  const mealsUpside = totalMeals * 0.5; // the other 50% if reclassified as staff
  const cashUpside = cashWithdrawals.reduce((s, t) => s + (t.withdrawal ?? 0), 0);
  const uncatUpside = uncategorised.reduce((s, t) => s + (t.withdrawal ?? 0), 0);

  console.log(`  5. POTENTIAL ADDITIONAL DEDUCTIONS (if properly documented):`);
  console.log(`     → Meals reclassified as staff welfare:      +S$${mealsUpside.toLocaleString("en-SG", { minimumFractionDigits: 2 })}`);
  console.log(`     → Cash withdrawals with receipts:           +S$${cashUpside.toLocaleString("en-SG", { minimumFractionDigits: 2 })}`);
  console.log(`     → Uncategorised with business justification: +S$${uncatUpside.toLocaleString("en-SG", { minimumFractionDigits: 2 })}`);
  console.log(`     ──────────────────────────────────────────────`);
  console.log(`     → MAX ADDITIONAL DEDUCTIONS:                +S$${(mealsUpside + cashUpside + uncatUpside).toLocaleString("en-SG", { minimumFractionDigits: 2 })}`);
  ...
  fs.writeFileSync(outputPath, JSON.stringify({
    ...
    summary: {
      2023: buildYearSummary(allTransactions, 2023),
